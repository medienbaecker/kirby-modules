<template>
  <k-section :headline="headline" :buttons="sectionButtons">
    ...
    <k-draggable v-else-if="!isLoading" :list="modules" :options="dragOptions" @sort="onSort" class="k-modules-list">
      <div v-for="module in modules" :key="module.id" class="k-module" :data-module-id="module.id"
        :data-selected="selectedModule === module.id"
        :data-disabled="isDisabled(module)" :data-hidden="module.status === 'draft'"
        :tabindex="isDisabled(module) ? null : 0" @focus="select(module)" @focusin="select(module)">
        <div class="k-module-body" :data-collapsed="!isExpanded(module.id)">
          <header class="k-module-header">
            ...
            <k-drawer-tabs :tab="activeTabName(module)" :tabs="drawerTabs(module)"
              @open="switchTab(module, $event)" />
          </header>
          <div v-if="isContentReady(module.id)" class="k-module-content">
            <k-sections v-for="tab in module.tabs"
              v-show="isExpanded(module.id) && activeTabName(module) === tab.name" :key="tab.name"
              :parent="pageUrl(module.id)" :tab="tab" :content="currentValues(module.id)"
...
.k-module {
  ...
  &[data-hidden="true"] .k-module-body {
    opacity: 0.5;
  }
